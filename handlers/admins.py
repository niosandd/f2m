...
def admin_menu():
    ...
    btn1 = InlineKeyboardButton(text="⛔️СТОП-ЛИСТ", callback_data="admin_stop_list")
    ...
    btn5 = InlineKeyboardButton(text="🗯️РАССЫЛКА", callback_data="admin_notification")
    btn6 = InlineKeyboardButton(text='↩️ВЕРНУТЬСЯ НА ГЛАВНУЮ', callback_data='return_to_choice_of_restaurant')
    ...
    try:
        ...
        for waiter in stats:
            if None not in waiter:
                ...
                text += (
                    f"{waiter[1]} {waiter[2]} {waiter[3]}\nКоличество уникальных заказов: <b>{str(len(set(eval(waiter[4]))))}</b>"
                    f"(<b>{str(current_waiter_count)}</b>)\n\n")
...
async def admin_commercial(call: types.CallbackQuery):
    try:
        ...
        text = (f"Статистика вовлеченности пользователей к заведению:\n\n"
                f"Кликабельность заведения через поиск: <b>{str(total_click_count)} ({str(current_click_count)})</b>\n\n"
                f"Обслужено гостей через f2m: <b>{str(total_guests_count)} ({str(current_guests_count)})</b>\n"
...
async def go_to_review(call: types.CallbackQuery):
    ...
    if review[0] and review[0] != "None" and review[1] and review[1] != "None":
        text += f"📅 <b>Дата и время:</b>   {review[0].split()[1]}  {format_date(review[0].split()[0])}\n"
        text += f"🍽️ <b>Блюдо:</b>   {review[1]}\n"
        if review[2] and review[2] != "None":
            rating_stars = "⭐" * review[2]
            text += f"🌟 <b>Оценка:</b>   {rating_stars} ({review[2]}/5)\n"
        if review[3] and review[3] != "None":
            text += f"💬 <b>Мнение:</b>   \"{review[3]}\"\n\n"
    if temp_review == 0:
        await bot.edit_message_text(chat_id=admin_id,
                                    message_id=call.message.message_id,
                                    text=text,
...
async def admin_reviews(call: types.CallbackQuery):
    ...
    if len(reviews) > 1:
        await bot.edit_message_text(chat_id=admin_id,
                                    ...
                                    text=text,
                                    reply_markup=InlineKeyboardMarkup().row(
                                        InlineKeyboardButton(text="Следующий отзыв", callback_data="go_to_next_review"),
                                        InlineKeyboardButton(text="Назад", callback_data="back_to_admin_menu")))
    else:
        await bot.edit_message_text(chat_id=admin_id,
                                    message_id=call.message.message_id,
...
async def return_to_choice_of_restaurant(call: types.CallbackQuery):
    ...
    message_obj = await bot.edit_message_text(chat_id=admin_id,
                                              ...
                                              reply_markup=InlineKeyboardMarkup().add(
                                                  InlineKeyboardButton(text="🔎 Поиск кафе",
