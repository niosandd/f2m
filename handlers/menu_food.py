    ...
    if dish is not None:
        ...
        if dish['Ресторан'] == "Блан де Блан":
            text = (f"🍤 <b>Кафе:</b>\n"
                    ...
                    f"<b>Если блюдо тебя заинтересовало, не забудь его добавить в корзину!</b>")
        else:
            text = (  # f"🍤 <b>Кафе:</b>\n"
                # f"<i>«{dish['Ресторан']}», {dish['Адрес']}</i>\n"
                # f"\n"
...
async def check_order(call: types.CallbackQuery):
    try:
        ...
        text = "❗️Проверь корзину, перед тем как сделать заказ ❗️\n\n" \
               "<i><b>Нажми на позицию</b>, чтобы убрать ее из корзины или <b>вернись к категориям</b>, чтобы добавить еще блюда</i>"
        await bot.edit_message_text(
            chat_id=user,
            message_id=call.message.message_id,
            text=text,
